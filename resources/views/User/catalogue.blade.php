@include('User/head')
                    ...
                    <div id="kt_header" class="header" data-kt-sticky="true" data-kt-sticky-name="header" data-kt-sticky-offset="{default: '200px', lg: '300px'}">
                        ...
                        <div class="container-fluid d-flex align-items-stretch justify-content-between" id="kt_header_container">
                            ...
                            <div class="page-title d-flex flex-column align-items-start justify-content-center flex-wrap me-2 mb-5 mb-lg-0" data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', lg: '#kt_header_container'}">
                                <!--begin::Heading-->
                                <h1 class="text-dark fw-bolder mt-1 mb-1 fs-2">{{__('Company catalog')}}
                            ...
                            <div class="card">
                                <div class="card-body"> 
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <h4 class="mb-2">{{__('List of companies registered for the International Cotton Show')}} </h4>
                                        <p class=" text-dark fw-bold fs-4">
                                            {{__('Review the catalog and add new appointment requests')}}             
                                         </p>
                                        <!--end::Text-->
                                           <div class="select-box" style="margin-left : 0px; margin-top :0px;">
                                        <form action="/filtre_catalogues" method="get" class="row g-3" style="padding : 0px;">
                                            <div class="col-md-3">
                                               <select name="filtre_catalogues_pays" class="form-select" aria-label="Default select example">
                                                   <option value="">{{__('Select by country')}}</option>
                                                   @php 
                                                   $pays_inside = array();
                                                   $enterprises = DB::table('entreprises')->select('pays')->get();
                                                   foreach($enterprises as $enterprise)
                                                    if(!in_array($enterprise->pays, $pays_inside))
                                                         array_push($pays_inside, $enterprise->pays);
                                                   @endphp
                                                    <?php $pays = DB::table('pays')->orderBy('libelle_fr', 'asc')->get(); ?>
                                                    
                                                   @foreach($pays as $pa)
                                                       @if(in_array($pa->libelle_fr,$pays_inside))
                                                       <option value="{{$pa->libelle_fr}}">{{$pa->libelle_fr}}</option>
                                                       @endif
                                                   @endforeach
                                               </select>
                                           </div>
                                        ...
                                        <div class="col-md-3">
                                           ...
                                           <select name="secteur_a" class="form-select" aria-label="Default select example">
                                               <?php $secteurs = DB::table('secteur_activites')->orderBy('libelle', 'asc')->get(); ?>
                                               <option value="">{{__('Select by sector')}}</option>
                                         ...
                                         <div class="col-md-3">
                                           ...
                                           <select name="profile_entreprise_a" class="form-select" aria-label="Default select example">
                                               <?php $profils = DB::table('profils')->orderBy('libelle', 'asc')->get(); ?>
                                               <option value="">{{__('Select by profile')}}</option>
                                               @foreach($profils as $profil)
                                               <option value="{{$profil->libelle}}">{{$profil->libelle}}</option>
                                               @endforeach
                                           </select>
                                        </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <a href="{{route('home.catalogue')}}" style="padding:6px 5px; background:#480099; color:white " class="btn btn-default">{{__('Refresh list')}}♻️</a><br><br>
        
                                    <input class="form-control" type="search" id="search_text" placeholder="Rechercher une Entreprise"> <br>
                                    <div class="tables">
                                        <div class="table-responsive">
                                        <table id="datatable-buttons" style ="width: 100%;" class="table table-hover table-striped border gy-7 gs-7">
                                            <thead>
                                            ...
                                            <tbody id="autocomplete">
                                                ...
                                                <tr>
                                                    ...
                                                    <td><a href="{{route('showEntreprise', $entreprise->id)}}">{{$entreprise->pays}}</a></td>
                                                        ...
                                                        <form method="POST" action="{{route('ajouter.suggestion', $entreprise->id)}}">
                                                            {{ csrf_field() }}
                                                              @if(in_array($entreprise->id, $input))
                                                                <input type="submit" id="p1" style="background: #7f8c8d; color:white" class="btn btn-default" value="Rendez-vous déja demandé" disabled></button>
                                                              @else 
                                                                <input type="submit" id="p1" class="btn btn-primary" value="Demander un rendez-vous" data-bs-toggle="tooltip" data-bs-placement="top" title="Cliquer ici pour envoyer une demande de rendez-vous à cette entreprise. Aller dans 'Mes Rendez-vous' pour visualiser vos différents souhais de rendez-vous en attente de confirmation par les entreprises concernées."/>
                                                              @endif 
                                                        </form>
   ...
   <!-- Request Catalogue with A.J.A.X by MAX BIRD  -->
        ...
        $(document).ready(function(){
            $('#search_text').keyup(function(){
                ...
                if(txt){
                    ...
                    $('#autocomplete').html("<td colspan='4' style='color:#6C757D'>Typing ... </td>");          
            });
        });
    </script>
