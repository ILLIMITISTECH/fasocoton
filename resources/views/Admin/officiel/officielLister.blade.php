       ...
       @include('Admin/Dashboard.sideBarUser')
        ...
        <div class="main-panel">
          ...
          <div class="bd">
             ...
             <h4 class="card-title"style="margin-left:30px;"> <br>Liste des Officiels</h4>
          </div>
                        ...
                        <table class="table">
                          ...
                          @foreach ($officiels as $officiel)
                            <tr>
                                ...
                                <td>{{$officiel->nom}}</td>
                                <td>{{$officiel->prenom}}</td>
                                <td>{{$officiel->fonction}}</td>
                                <div class ="option-column">
                                <td class="text-center"> 
                                
                                <a href="{{route('officiels.edit', $officiel->id)}}">
                                 <button type="button" class="btn  btn-sm"style="background:#23B40B;color:white"><i class="bi bi-pen-fill"></i></i></button>
                                 </a>
                               
                                <form action="{{route('officiels.destroy', $officiel->id)}}" method="post">
                                  {{ csrf_field() }}
                                  @method('DELETE')
