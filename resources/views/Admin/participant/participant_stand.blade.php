        ...
        <div class="main-panel">
          ...
          <div class="bd">
              <?php
                        ...
                        @if($even->status == 1)
             <h4 class="card-title"style="margin-left:30px;"> <br>Liste des participants inscrites au {{$even->nom_event_fr}} avec Stand</h4><br><br><br>
                      @else
                          ...
                          @foreach ($participant as $participants)
                            <tr> 
                                ...
                                @elseif($participants->presence == 2)
                                 ...
                                 <td>--</td>
                                @endif
                                <td>{{$participants->village}}</td>
                                @if($participants->village == "VILLAGE PME") 
                                <td>{{$participants->type_village}}</td>
                                @elseif($participants->village == "VILLAGE FINANCE")
                                <td>{{$participants->type_village1}}</td>
                                @elseif($participants->village == "VILLAGE PAYS")
                                <td>{{$participants->type_village2}}</td>
                                @else
                                <td>--</td>
                                @endif
                                <td class="text-center"> 
                                
                                <a href="{{route('stand.edit', $participants->id)}}">
                                <button type="button" class="btn  btn-sm"style="background:#23B40B;color:white"><i class="bi bi-pen-fill"></i></i></button>
                              </a><br><br>
                                
                                <form action="{{route('destroy_participantStand', $participants->id)}}" method="post">
                                  {{ csrf_field() }}
                                  @method('DELETE')
