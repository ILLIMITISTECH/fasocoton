        ...
        <div class="main-panel">
          ...
          <div class="bd">
              <?php
                        ...
                        $exposant = DB::table('exposants')->get();
                        ?>
                        @foreach($evens as $even)
                        @if($even->status == 1)
             <h4 class="card-title"style="margin-left:30px;"> <br>Liste des exposants inscrites à {{$even->nom_event_fr}} : {{count($exposant)}}</h4>
                      @else
                    ...
                    <div class="card-body">
                      ...
                      <div class="table-responsive">
                        <table class="table">
                          ...
                          @foreach ($exposant as $exposants)
                            <tr>
                                
                                <td>{{$exposants->nom}}</td>
                                <td>{{$exposants->prenom}}</td>
                                <td>{{$exposants->email}}</td>
                                <td>{{$exposants->phone}}</td>
